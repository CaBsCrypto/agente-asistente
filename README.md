...
| User identity and wallet | Privy login + automatic user-owned Stellar wallet |
...
Stellar is not one of Privy's client-side automatic wallet chains, so the
server performs an idempotent `get-or-create` after verifying the Privy access
token. A deterministic external ID and Privy idempotency key prevent duplicate
wallets during retries or concurrent onboarding requests.

There is no password generated by agent-assistant and no seed phrase shown
at signup. Privy's authenticated user is the wallet owner. Login establishes
identity only; future payments must still pass spend policy and obtain a
transaction-scoped wallet authorization.
...
| Privy account + automatic Stellar wallet | Implemented | Public `/agent` |

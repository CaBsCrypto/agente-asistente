// The MCP server for the provider surface, kept out of the route file so tests
// can drive it directly. See app/mcp/agent-server.ts for why that matters.
...
export const PROVIDER_MCP_ENDPOINT = "/api/mcp/provider";
let handler: ReturnType<typeof createMcpHandler> | null = null;

export function getProviderMcpHandler() {
  return (handler ??= createMcpHandler(
    ...
    {
      // See the note on the agent route: basePath derived /api/mcp/mcp, which
      // no file serves, so every authenticated request 404'd.

...
const createSchema = z.object({ name: z.string().trim().min(1).max(80).optional(), scopes: z.array(z.enum(PERSONAL_MCP_SCOPES)).min(1).optional(), expiresInDays: z.number().int().min(1).max(365).optional() });
...
function errorResponse(error: unknown) {
  ...
  const status = code === "invalid_personal_mcp_token_request" || code === "personal_mcp_expiration_invalid" || code === "personal_mcp_scope_invalid" ? 400 : code === "invalid_origin" ? 403 : code === "database_not_configured" ? 503 : 401;

...
export default function Home(){return <main>
  <nav className="nav shell"><a className="brand" href="#top"><b>AA</b>agent-assistant</a><div><a href="#product">Product</a><a href="#pilots">Pilots</a><a href="/developers">Developers</a><a href="/connections">Integration Lab</a><a href="/waitlist">Waitlist</a></div></nav>
  <section className="hero shell" id="top"><div><p className="eyebrow">COMMERCE INFRASTRUCTURE FOR AI AGENTS</p><h1>Agents can take action. <em>You stay in control.</em></h1><p className="lede">A non-custodial control layer for agents that discover, book, hire and pay under policies defined by real people.</p><div className="actions"><a href="/waitlist">Waitlist</a><a href="/connections">Explore integrations</a></div><small>Private beta waitlist open | Live MCP sandbox | No real funds enabled</small></div>
  ...
  <section className="section shell" id="pilots"><p className="eyebrow">INITIAL ACTION NETWORK</p><h2>Three pilots. One reusable architecture.</h2><div className="pilots">{pilots.map((p,i)=><article className={i===0?"hot":""} key={p[1]}><span>{p[0]}</span><h3>{p[1]}</h3><p>{p[2]}</p></article>)}</div></section>
